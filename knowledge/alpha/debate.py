...
def generate_alpha_debate_prompt(
    ...
    prompt = (
        ...
        "### 最终判决\n\n"
        "基于辩论结果，给出明确的行动指引：\n\n"
        "**重要：不要因为双方都有道理就默认选择「势均力敌」。**"
        "LLM 天然倾向折中——这恰恰是裁判最需要克服的偏差。"
        "如果一场辩论的结论是「双方都有道理」，那这场辩论就没有产生信息增量。"
        "你的工作是**站队**：谁的证据更硬、逻辑链更完整、对关键假设的检验更充分？"
        "如果确实势均力敌，这本身就是高不确定性信号——应倾向马克斯（降低仓位），"
        "因为不确定性高时，等待的 option value > 行动的 alpha。\n\n"
        "- **谁更有说服力**：索罗斯 / 马克斯（必须二选一，附理由）\n"
        "- **debate_conviction_modifier**：给出 0.5-1.5 的信念调整系数\n"
        "  - 0.5-0.7：马克斯完胜，大幅降低仓位\n"
        "  - 0.8-0.9：马克斯略胜 或 不确定性高，谨慎降低\n"
        "  - 1.0：仅当辩论未触及核心假设时使用（需明确解释为什么辩论没有改变任何判断）\n"
        "  - 1.1-1.2：索罗斯略胜，适度加码\n"
        "  - 1.3-1.5：索罗斯完胜，大幅加码\n"
