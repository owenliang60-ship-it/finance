...
## 待部署修复：live 身份继承与调仓周末延后（2026-09-25）

以下行为在 `codex/index-pe-live-issuer-identity` 实现，尚未部署；当前源数据阻塞见 `docs/audit/2026-09-25-index-pe-live-issuer-identity.md`。

- 调仓后首个工作日 20:00 UTC 之前，若价格日历恰好停在调仓收盘日，来源阶段显式返回 `live_deferred`，保留旧构成供仍属旧窗口的估值使用。过该保守截止时间仍无下一交易日价格则失败；假日及冬令时可能提前拒绝手动执行，不自动延长宽限。
- live 的发行人可继承同篮子最近一份 PIT 可用披露中的精确 CUSIP 证据；先选完整快照，再查证券，不回退更早披露。缺失证据允许有效人工 override；CUSIP/ISIN 或发行人冲突保持拒绝。验证端独立重建。
