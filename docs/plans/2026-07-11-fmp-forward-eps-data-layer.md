...
**Status:** Ready for Boss annotation after three technical-review rounds; do not implement before approval.
...
**Goal:** 在不破坏现有 yfinance 周频线的前提下，上线 FMP quarterly forward EPS 数据层：Spec 4 张业务表 + 1 张最小 run-manifest 审计表、5 个 ETF holdings 快照、约 1,050–1,150 股票周频 PIT 快照、2021+ 一次性 backfill、独立 verifier、单一 `finance_forward` cron 串行编排与安全 key 轮换。
...
- [ ] 周频 universe 精确等于 `extended_pool ∪ 5 baskets included normalized symbols ∪ MAGS 7`，verifier 使用同一个 resolver 作为分母。
...
7. 任何真实 key 只通过已存在的本地 `.env` staging 变量和人工安全写入传递；命令、文档、commit message 不出现值。
...
def resolve_fmp_forward_universe(
    ...
    return sorted({s.upper() for s in extended_symbols} | included |
...
3. full universe loader is `get_extended_symbols()` (includes core), never `get_extended_only_symbols()`;
...
3. normal/dry-run path: resolve the **full** universe using `get_extended_symbols()` ∪ included holdings ∪ MAGS;
