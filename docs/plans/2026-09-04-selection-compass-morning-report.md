...
- [ ] Stop D canary run complete，125 个 dataset jobs 全终态，`fetch_failed <5%`，数据库 `quick_check=ok`。
- [ ] Full backfill run complete，独立 `--verify-only` 通过；三表覆盖和 8Q continuity 均 ≥95%，profile ≥98%，所有缺口有显式 attribution。
- [ ] `compute_all_metrics` 完成；命令显式 `assert not failures`，并断言返回结果与 exact current-base 的交集覆盖 ≥95%，任一失败非零退出。
- [ ] EPS YoY 与 QoQ 必须同时通过；任一不通过则不命中。
- [ ] `negative/zero comparison → positive current` 显示“扭亏”；`negative → less negative` 不命中。
- [ ] 四季度成长严格使用两个现有 CAGR 的算术平均且阈值 15%。
- [ ] 最近 7 个交易日任一天 RVOL ≥2σ 即通过，并显示窗口最高值与对应日期。
- [ ] 输出包含当前市值和 β6M，并严格按市值从高到低。
- [ ] fundamental-ready 与 RVOL-ready 都以 exact Extended 为分母、分别 ≥95%；subtitle 显示两个 `covered/total`。任一不足时 warning + hits 为空。
- [ ] resolver 异常/空 universe 时 unavailable，绝不回退 Core/raw JSON。
- [ ] stale fundamental/status、stale price、stale market cap 均不能进入榜单或冒充当前值。
- [ ] raw EPS/CAGR 输入缺失、YoY fiscal-period 不匹配或季度间隔断裂计入 fundamental not-ready；真实存在但非正的 CAGR 基期计入 ready、业务不通过。
- [ ] Text、HTML、PNG/PDF 三面列完全一致。
- [ ] Targeted tests 全绿；全量测试相对 main 零新增失败。
...
- [ ] **Step 1:** Production preflight: HEAD, active denominator, disk, writer processes, `quick_check`, zero existing run.
- [ ] **Step 2:** Run `canary-2026-09-04` with `--canary 25`.
- [ ] **Step 3:** Run `--verify-only` against that exact run id; inspect per-dataset status distribution and provider-empty/fetch-failed lists.
- [ ] **Step 4:** If canary passes, start fixed `full-2026-09-04` run under the runner's internal `market_db_writer` flock.
- [ ] **Step 5:** Wait on the exact process/run id; do not restart on observation timeout.
- [ ] **Step 6:** After terminal completion, run `--verify-only`, `verify_fundamental_coverage.py --json`, `PRAGMA quick_check`, and inspect DB growth.
- [ ] **Step 7:** Run metrics recomputation under `cron_wrapper` with resource key `market_db_writer`; Python 命令必须 `assert not failures`，并断言 exact current-base metrics coverage ≥95%，否则非零退出。
- [ ] **Step 8:** Pull the authoritative DB locally and re-run coverage proof.
...
- [ ] **Loop 1 — EPS RED:** Add `_eps_leg` tests for +25%, below 25%, positive turnaround, zero-to-positive, loss narrowing; run exact test and observe import/missing-function failure.
- [ ] **Loop 1 — EPS GREEN:** Implement only `_eps_leg`; rerun exact tests to green.
- [ ] **Loop 2 — AND RED:** Add fixture proving YoY and QoQ are AND; run and observe failure.
- [ ] **Loop 2 — AND GREEN:** Implement only EPS-pair evaluation; rerun green.
- [ ] **Loop 3 — CAGR RED/GREEN:** Add exact arithmetic-average, threshold boundary and missing-CAGR tests; implement only CAGR predicate; rerun green.
- [ ] **Loop 4 — RVOL RED/GREEN:** Add any-day recent-7 max/date and insufficient/stale-frame tests; implement with existing `calculate_rvol_series`; rerun green.
- [ ] **Loop 5 — Raw readiness RED/GREEN:** Add tests for current/prior/YoY EPS NULL, missing YoY period+prior-FY row, missing CAGR endpoint, and >`FUNDAMENTAL_QUARTER_GAP_MAX_DAYS` quarter gap → fundamental not-ready. Add the contrasting case where all raw values exist but net-income base `<=0` → ready yet CAGR business predicate fails. Implement raw-input readiness only; rerun green.
- [ ] **Loop 6 — Coverage RED/GREEN:** Add exact Extended denominator, income status, stale fundamental, metrics-date mismatch, separate fundamental/RVOL readiness and `<95%` fail-closed tests; implement coverage payload; rerun green.
- [ ] **Loop 7 — Market Cap RED/GREEN:** Add as-of observation-date freshness, missing hit cap fail-closed, market-cap DESC/tie-break tests; implement sorting; rerun green.
- [ ] **Step 7:** Run full `tests/test_selection_compass.py`; expect all pass.
- [ ] **Step 8:** Commit exact files with `feat(morning): add selection compass screen engine`.
...
- [ ] **Loop 1 — Strict universe RED/GREEN:** Test resolver success passes exact active symbols; resolver error/empty returns unavailable and never passes fallback `pool_symbols`; implement strict branch; rerun green.
- [ ] **Loop 2 — as-of inputs RED/GREEN:** Test existing price frames plus one price-derived as-of and dated market-cap observations reach scanner; implement minimal wiring; rerun green.
- [ ] **Loop 3 — Beta RED/GREEN:** Test Beta is computed only for compass hits and injected; implement reuse of `_compute_signal_betas`; rerun green.
- [ ] **Loop 4 — Payload RED/GREEN:** Test result lands at `market_signals["selection_compass"]` without changing PMARP/volume outputs; implement return field; rerun green.
- [ ] **Step 5:** Run builder-related morning tests; expect pass.
- [ ] **Step 6:** Commit exact files with `feat(morning): wire selection compass into daily scan`.
...
- [ ] **Loop 1 — Text RED/GREEN:** Add percent/turnaround/cap/Beta/RVOL/date/order/coverage-subtitle tests; implement text formatter; rerun green.
- [ ] **Loop 2 — HTML RED/GREEN:** Add exact heading/columns/rows/order test; implement HTML block after 0b; rerun green.
- [ ] **Loop 3 — Visual RED/GREEN:** Add exact 10-column widths/cells/order test; implement visual section before PMARP; rerun green.
- [ ] **Loop 4 — Visibility RED/GREEN:** Add healthy-no-hit hidden and unhealthy-coverage warning tests across surfaces; implement visibility helper shared by all surfaces; rerun green.
- [ ] **Step 5:** Run all morning report tests.
- [ ] **Step 6:** Commit exact files with `feat(morning): render selection compass across delivery surfaces`.
...
- [ ] **Step 1:** Run `pytest tests/test_selection_compass.py tests/test_morning_report.py -q`.
- [ ] **Step 2:** Run full `pytest tests/ -q`; compare exact failures with baseline main.
- [ ] **Step 3:** Run local real-DB dry build; assert denominator/coverage/hits and descending market caps.
- [ ] **Step 4:** Run HTML compile and inspect generated file contains heading and exact columns.
- [ ] **Step 5:** Run real PNG render and PDF composition; inspect title, 10 columns, no zip truncation, market-cap row order. Force HTML send failure and prove produced fallback PDF includes the same section.
- [ ] **Step 6:** Plan/spec compliance review, code-quality review, QA evidence review; fix findings with RED→GREEN tests.
