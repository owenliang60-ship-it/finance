...
| 存量 drift：HTML PMARP 缺"变化"列（文本/PNG 都有），Task 4 顺手补齐 + parity 测试 | `morning_report.py:1443` vs `:1570` |
| 4 个存量 builder 测试直调 `build_market_signal_report()`，接线后需 patch `_compute_signal_betas` 隔离 DB | `tests/test_morning_report.py:1204/:1253/:1302/:1450` |
...
(e) **修补存量 builder 测试隔离 DB（plan review P1）**：4 个 `build_market_signal_report()` 调用点（`tests/test_morning_report.py:1204/:1253/:1302/:1450` 所在测试，均已有 `monkeypatch` fixture）各加一行，避免接线后 `_compute_signal_betas` 真实读 market.db（`get_store()` 初始化会创建/打开 DB，`market_store.py:551`）：
...
def test_pmarp_columns_exact_parity_across_three_surfaces():
    ...
    修复存量 drift：HTML 此前缺'变化'列（morning_report.py:1443 vs :1570）。"""
...
```bash
"/Users/owen/CC workspace/Finance/.venv/bin/pytest" tests/test_morning_report.py -k "beta or parity" -v
```
Expected: 新增 3 个 FAIL（β6M 列缺失 ×2 + PMARP 三面 parity）

- [ ] **Step 4.3: 实现**

(a) HTML `build_html_payload`（`:1443-1451`）——顺手修存量 drift：HTML 此前缺文本/PNG 都有的"变化"列（plan review P2），一并补齐恢复 one-to-one：
...
for sym in ["NVDA", "MSFT", "KO"]:
    # auto_adjust=False 对齐项目现有口径（broad_market_scan.py:355、extended_price_fetcher.py:159）
    raw = yf.download([sym, "SPY"], period="9mo", progress=False, auto_adjust=False)["Close"].dropna()
...
```
Expected: 与 main 基线一致 + 新增 21 个测试（Task 1-4: 8+6+4+3）全 passed（worktree 基线注意 memory `project_worktree_needs_live_data_symlink`：缺 live data 时部分 registry 测试结果会漂移，对照 main 跑一次基线确认无新增失败）。
...
- **Spec 覆盖**：§1 口径（126d/SPY/日频）→ Task 1；§3.1 模块（`compute_beta`，`get_beta_6m` 已按 2026-06-12 review 从 spec 移除，YAGNI）→ Task 1；§3.2 接线/单次加载 SPY/只算命中股 → Task 2；§3.3 双路径渲染 + 缺失 `—` → Task 3/4（实为三路径：文本/HTML/PNG，PNG 是 PDF fallback 的来源，三面 one-to-one 是代码硬约束）；§5 数据不足/错位/3.10/性能 → Task 1 测试 + Task 6.2；§6 验收 1/2/3/4 → Task 1 / 5.1 / 5.2 / 5.3；§7 范围外未引入。
...
## Plan Review 修订记录（2026-06-12，Boss 静态审阅）

| Finding | 处理 |
|---------|------|
| [P1] builder 集成路径未测 + 存量 builder 测试接线后穿透真实 DB | Task 2 增加 `test_builder_injects_beta_into_signal_rows`（patch `_compute_signal_betas` 隔离 DB，断言 beta 进入 pmarp/volume_anomaly rows）；Step 2.3(e) 给 4 个存量 builder 测试加同款 patch |
| [P2] HTML PMARP 缺"变化"列，与"三面 one-to-one"表述冲突 | 选补列方案：Task 4 HTML 加"变化"列恢复 parity，新增 `test_pmarp_columns_exact_parity_across_three_surfaces` exact 断言 |
| [P2] spec 状态/`get_beta_6m` 漂移 | spec 状态改"已批准"；`get_beta_6m` 从 spec 移除（本轮复用 price_frames，无第二消费方）|
| 小修 | yfinance 对照加 `auto_adjust=False`（对齐 `broad_market_scan.py:355`）；测试计数修正为 21（8+6+4+3）|
