...
采纳方案 **A+**（substring `keyword_rules` 整体重建为 FMP `(sector,industry)→(l1,l2)`
精确查表），详见 `docs/plans/2026-05-16-concept-rule-classifier-rebuild.md`。

- **代码** `c78297f`：`concept_taxonomy_v2.json` 删 `keyword_rules`、加 82-pair
  `industry_map`；`ConceptRegistry.classify()` 改精确查表（命中即 `rule`，歧义
  industry 缺席→`unclassified`→LLM）；新增 L2 `telecom_operator`（SSOT 11/61/42）；
  builder 加 `--reclassify` 重跑入口。
- **数据** `1671e1d`：Task 7 `--reclassify` 重跑 + Task 8 人工裁决产出
  `extended_pool_tags_2026-05-16.csv`（545 行，rule 289/llm 249/manual 7）。
- **落库** 2026-05-17：云端 `market.db` `company_concept_tags` 545 行；抽查确认
  BRK-B→金融/保险、SO/DUK→地产与公用/电力与公用、VZ/T/TMUS→互联网与软件/电信运营商，
  原 30 个跨 sector 误命中全部归正。
- 确定性查表零顺序敏感、可审计；语义抽检（Task 8）保留为第二道闸。
