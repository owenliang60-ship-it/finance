...
- **#1 错误吞没（代码 `c78297f`）**：`llm_concept_prefill.py:_run_claude_cli` 失败时
  同时记 `stdout`（`claude -p --output-format json` 把错误打到 stdout，原来只记
  stderr 等于丢诊断）。已修。
- **#4 重跑入口**：builder 新增 `--reclassify <csv>` 模式 —— 读旧 CSV 逐行重算并
  写新 CSV，长流程任何环节失败都能复跑（见 plan §3.7）。
- **实跑验证**：Task 7 `--reclassify` 重跑只对「industry 不在 map」的旧 rule 行花
  fresh LLM，实测 **69 次、0 失败、≈$14**（远低于全量 533 的 ~$70），未撞限流。
- **#2 SDK + prompt caching 改造**：**有意推迟**（plan §7 范围边界）—— 69 次的量
  不足以 justify 重构，留作未来扩展池规模上来后再做。这一项是 known-deferred，
  不是回归。
